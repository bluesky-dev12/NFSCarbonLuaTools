...
struct Function_ {
	...
	int Rvar[MAXARG_A];
	/* A stripped-chunk local whose first assignment still needs the `local`
	 * keyword.  Keeping this separate lets us emit `local x = expr` instead of
	 * a synthetic `local x` followed by `x = expr`, which also preserves the
	 * original Black Box instruction shape much more closely. */
	int RsyntheticNew[MAXARG_A];
	...
	int Rpend[MAXARG_A];
	/* Bytecode PC that produced the currently pending register expression. */
	int Rorigin[MAXARG_A];
	...
	int elseWritten;
	int testjump;
	/* Carbon gameplay chunks are stripped of debug upvalue names.
	 * Keep names inferred from the parent CLOSURE capture descriptors here. */
	char* inferredUpvalues[32];
};
